...
class Misc:
    ...
    def LinePlane(line, triangle, cp=True):
        try:
            ...
            n = Vector3.Cross(u, v)
            r = (Vector3.Dot(n, Vector3.Subtract(triangle.points[
                 0], line.start)) / Vector3.Dot(n, line.direction))
            if stp:
                point = Vector3.Add(Vector3.Scale(
                    line.direction, r), line.start)
    ...
    def LP2(line, triangle, cp=True):
        try:
            ...
            u = Vector3.Subtract(
                triangle.points[1].point, triangle.points[0].point)
            v = Vector3.Subtract(
                triangle.points[2].point, triangle.points[0].point)
            n = Vector3.Cross(u, v)
            d = Vector3.Subtract(line[1], line[0])
            r = (Vector3.Dot(n, Vector3.Subtract(
                triangle.points[0].point, line[0])) / Vector3.Dot(n, d))
            if cp:
    ...
    def Phong(normal, viewer, light, material, term):
        ...
        if ldn > 0:
            ...
            rdv = Vector3.Dot(Vector3.Subtract(
                Vector3.Scale(n, 2 * ldn), l), v)
            if rdv > 0:
                val += (material[2][term] *
                        (rdv**material[3]) * light[2][term])
        # print(val)
        return val
        ...
        for light in lights:
            for n in range(3):
                val[n] += Misc.Phong(triangle.InterpolatedNormal(coord),
                                     ...
                                     (light.To(intersect[2]), light.Diffuse(
                                         intersect[2]), light.Specular(intersect[2])),
                                     (Color(),
                                      Color.Multiply(triangle.material.color[
                                                     'diffuse'], triangle.Map('diffuse', coord)),
                                      Color.Multiply(triangle.material.color[
                                                     'specular'], triangle.Map('specular', coord)),
    ...
    def Ray(start, direction, scene, color=True, sector=None):
        ...
        for triangle in scene.triangles:
            ...
            if possible:
                tmp = Misc.LP2(
                    [start, Vector3.Add(start, direction)], triangle, color)
                write = False
        ...
        if color and (intersect != None):
            ...
            for light in scene.lights:
                block = Misc.Ray(intersect[2], light.To(
                    intersect[2]), scene, False)
                if block == None:
                    ...
                    if Vector3.Subtract(light.location, intersect[2]).Magnitude() < block[0]:
                        applicable.append(light)
            col = Misc.Lighting2(
                start, direction, scene.ambient, intersect, intersected, applicable)
...
class Vertex:
    ...
    def Transform(self, points, norms):
        self.point = Matrix2.Multiply(
            self.bpoint.Horizontal(), points).Vectorize()
        self.normal = Matrix2.Multiply(
            self.bnormal.Horizontal(), norms).Vectorize()
...
class Matrix2:
    ...
    def Multiply(A, B):
        if A.cols != B.rows:
            raise RuntimeError(
                'Column count of Matrix2 \"A\" does not match row count of Matrix2 \"B\".')
    ...
    def Vectorize(self):
        if self.cols == 1:
            if self.rows != 4:
                raise RuntimeError(
                    'Only 1 by 4 or 4 by 1 Matrix2s can be cast to Vector3s.')
            vertical = True
        elif self.rows == 1:
            if self.cols != 4:
                raise RuntimeError(
    ...
    def Inverse(self):
        ...
        if det == 0:
            raise RuntimeError(
                'Cannot find the inverse of a matrix with a determinant of 0')
    ...
    def Add(A, B):
        if A.rows != B.rows:
            RuntimeError(
                'The row counts of Matrix \"A\" and Matrix \"B\" are not identical.')
        if A.cols != B.cols:
            RuntimeError(
                'The column counts of Matrix \"A\" and Matrix \"B\" are not identical.')
...
for x in range(width):
    ...
    for y in range(height):
        l = Vector3(theta * 2 * ((x / width) - .5), theta *
                    2 * ((y / width) - .5), 1).Unit()
        ...
        if type(d) != type((-1)**.5):
            ...
            for i in range(3):
                ...
                light = Misc.Lighting([.1, .1, .1], normal, Vector3.Scale(
                    intersection, -1), lights, (vals, vals, [1, 1, 1], 10), i)
                if light > 1:
                    light = 1
                elif light < 0:
                    light = 0
                coloring.append(round(255 * light))
            screen.set_at((x, height - y),
                          pygame.Color(coloring[0], coloring[1], coloring[2]))
