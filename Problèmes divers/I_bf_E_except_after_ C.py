...
def simple_stats(url='http://www.puzzlers.org/pub/wordlists/unixdict.txt'):
    ...
    not_c_ie = len(
        {word for word in words if re.search(r'(^ie|[^c]ie)', word)})
    not_c_ei = len(
        {word for word in words if re.search(r'(^ei|[^c]ei)', word)})
...
def print_result(cei, cie, not_c_ie, not_c_ei):
    ...
    print('(To be plausible, one count must exceed another by %i times)' %
          PLAUSIBILITY_RATIO)
