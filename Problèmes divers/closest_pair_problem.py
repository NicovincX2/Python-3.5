...
# Note the use of complex numbers to represent 2D points making distance
# == abs(P1-P2)
...
def _closestPair(xP, yP):
    ...
    Pl = xP[:int(numPoints / 2)]
    Pr = xP[int(numPoints / 2):]
    Yl, Yr = [], []
    xDivider = Pl[-1].real
    for p in yP:
        if p.real <= xDivider:
...
def times():
    ...
    for f in functions:
        print('Time for', f.__name__, timeit.Timer(
            ...
            'from closest_pair_problem import %s, pointList' % f.__name__).timeit(number=1))
