...
def return_Pos(self):
    ...
    tmpX,tmpY=str((self.x-curroffX)*1.0/currscaleX ),str(-(self.y-curroffY)*1.0/currscaleY )

    fen.title('f(' + tmpX[ :tmpX.find('.') + 3 ] + ')=' + tmpY[ :tmpY.find('.') + 3])
...
def trace_Org(offX,offY,scaleX,scaleY):
    ...
    while n<tX:
        ...
        if (n - offX) / scaleX != 0 :
            img.create_text(n, offY+10, text= str((n - offX) / scaleX) , tag='org' )
    ...
    while n<tY:
        ...
        if (n-offY) / scaleY !=0:
            img.create_text(offX-10,n,text=str(-(n-offY) / scaleY) , tag='org')
        else:
            img.create_text(offX-10,n+10,text=str((n-offY) / scaleY) , tag='org')
...
def trace_M(miniX,maxiX,miniY,maxiY,fonction,x,y,deriv,pas,pasF):
    ...
    traceFx(str(fonction),miniX-1,maxiX+1,offX,offY,echelleX,echelleY,pasF)
    euler(miniX-1,maxiX+1,pas,x,y,deriv,echelleX,echelleY,offX,offY)
...
def run_T():
        ...
        try:
            trace_M(int(minXe.get()),int(maxXe.get()),int(minYe.get()),int(maxYe.get()) , Fxe.get(),float(xDe.get()),float(yDe.get()),derive.get(),float(pase.get()),float(pasFxe.get()) )
        except:
...
xDc,xDe,yDc,yDe,derive,derivc,pasc,pase=Label(fen,text="F  ("),Entry(fen),Label(fen,text=') = ') , Entry(fen),Entry(fen),Label(fen,text="F ' (x) ="),Label(fen,text='Pas = '),Entry(fen)
...
#Bloc 1
debB=50
Label(fen,text='=' * 15 + "Equation d'une courbe" + '=' * 15).place(x=tX+10,y=debB-25)
...
Label(fen,text="=" * 15 + "Méthode d'Euler" + "=" * 15).place(x=tX+10, y= debB-25)
...
#Bloc 3
debB=450
Label(fen,text="=" * 15 + 'Traçage des courbes' + "=" * 15).place(x=tX+10, y= debB-25)
