...
### Ce script est un traceur de fonctions mathématique, développé avec python et la bibliothèque graphique Tkinter ###

#################################################### Les imports #############################################################
from tkinter import *       # Importation de la bibliothèque graphique Tkinter
from math import *          # Importation des fonctions mathématique sin(), cos(), sqrt(), ...
from tkinter import messagebox  # Importation du module affichant diverses messageBoxes
import sys, os                # Module utilisé pour les éxeptions
###############################################################################################################################
...
class Repere(Canvas) :
    ...
    def __init__(self, width=600, height=400, color="#FFFFFF") :
        Canvas.__init__(self)                           # Construction du widget parent
    ...
    def trace_axe(self, xmax=10, xmin=-10, ymax=10, ymin=-10, grad=2) :
        ...
        if grad == 2 :  # Graduation partielle
            ...
            for x in range(npix_x+1, 0, int(npix_x/xmin) ) : self.create_line(x, npix_y-2, x, npix_y+5, tag="grad")
            for x in range(npix_x+1, self.width+1, int(npix_x/(-xmin)) ) : self.create_line(x, npix_y-2, x, npix_y+5, tag="grad")
            # Graduations sur l'axe x -> partie positive puis négative :
            for y in range(npix_y+1, 0, int(-npix_y/ymax) ) : self.create_line(npix_x-2, y, npix_x+5, y, tag="grad")
            for y in range(npix_y+1, self.height+1, int(npix_y/ymax) ) : self.create_line(npix_x-2, y, npix_x+5, y, tag="grad")
        if grad == 3 :  # Graduation complète
            arrow = NONE
            # Graduations sur l'axe x -> partie négative puis positive :
            for x in range(npix_x+1, 0, int(-npix_x/(-xmin)) ) : self.create_line(x, 1, x, self.height, fill="#dbd8d8", tag="grad")
            for x in range(npix_x+1, self.width+1, int(npix_x/(-xmin)) ) : self.create_line(x, 1, x, self.height, fill="#dbd8d8", tag="grad")
            # Graduations sur l'axe x -> partie positive puis négative :
            for y in range(npix_y+1, 0, int(-npix_y/ymax) ) : self.create_line(1, y, self.width, y, fill="#dbd8d8", tag="grad")
            for y in range(npix_y+1, self.height+1, int(npix_y/ymax) ) : self.create_line(1, y, self.width, y, fill="#dbd8d8", tag="grad")

        if grad != -1 :
            self.create_line(0, npix_y+1, self.width, npix_y+1, arrow=arrow, tag="axe")   # Création de l'axe x (des abscisses)
            self.create_line(npix_x+1, 0, npix_x+1, self.height, arrow=arrow, tag="axe")    # Création de l'axe y (des ordonnées)
    ...
    def trace_courbe(self, equa, color="#000000", mod="normal", step=10) :
        ...
        for i in range(1, self.width+1) :
            ...
            try :
                ...
                messagebox.showerror(str(sys.exc_info()[0]), "L'équation \""+equa+"\" semble erronée")    # On affiche un message d'erreur
                self.courbe.pop()       # On supprime la dernière entrée de la liste self.courbe, car elle est érronée
                break       # On sort de la boucle for
            if (-(self.height**2) < y_pix < (self.height**2)) :   res.append((i, y_pix))     # On n'enregistre que les résultats servant à l'affichage
        ...
        try :
            ...
            if (mod == "point") or (mod == "both") :
                for i in range(0, len(res)-1) :
                    if not res[i][0] % step :
                        self.create_oval(res[i+1][0]-2, res[i+1][1]-2, res[i+1][0]+2, res[i+1][1]+2, fill=color, tag="point_"+equa)
            return 1    # Retourne 1 si tout c'est bien passé sinon on retourne rien (None)
        except :    return None
...
#------------------------------ Partie traçage d'une nouvelle courbes ---------------------------#
    ...
    def tracer() :
        try :
            if (int(pas_en.get()) > 50) and (mod.get() != "normal") :    # Si le pas est trop gros, on demande à l'utilisateur si il veut qd même tracer la courbe
    ...
    def choix_color() :
        global color
        color = str(colorchooser.askcolor(title="Choisir la couleur de la fonction", color="#999999")[1])
        color_bt.configure(bg=color)
#------------------------------------------------------------------------------------------------#
...
#-------------------------------- Partie configuration du fenetrage -----------------------------#
    ...
    def config_window() :
        try :
            ...
            messagebox.showerror("Erreur !!!", "Une ou plusieurs données pour le fenètrage, ne sont pas des données numériques.\nVeuillez rectifier.")
...
\n\nRalala, j'ai tjs révé de faire un About de ce style.\nMerci\n\n
    ...
    def coords_write(event) :
        lb_coords.configure(text="x= %4.2f et y= %4.2f" % (rep.convert_pix_to_unit("x",event.x), rep.convert_pix_to_unit("y",event.y)) )
        rep.delete("croix")
...
#------------------------------------------------------------------------------------------------#
    ...
    rep.bind("<Button-1>", coords_write)    # Si on presse le bouton gch de la souris sur le canvas, ça nous donne les coords au pts cliqué
    ...
    Button(lbfr_rep, text="Mettre à jour", command=config_window).place(x=430, y=88)
    Button(lbfr_rep, text="Réinitialiser", command=reinitialisation).place(x=525, y=88)
            # Les RadioButtons & co
    grad = IntVar()
    grad.set(2)
    ...
    Button(fr_but_br, text="À propos", command=apropos).pack(side=LEFT, padx=5, pady=5)
    Button(fr_but_br, text="Quitter", command=root.destroy).pack(side=LEFT, padx=5, pady=5)
