...
def onInit():  # Création des axes de la fenêtre de traçage
    # Trace la courbe entrée en effaçant les autres
    global xmin, xmax, ymin, ymax, pasx, pasy, save, lst1, a, compteur, uxx, ymin, uyy, selected, xo
    selected = False
    a = 0
    h = 30
    m = 100
...
# Créé la table de valeur de l'enssemble des points de la courbe
def onCalc(fonction, couleur, xo, yo, uy, uxx, xmin, e):
    ...
    while a < 701:
        ...
        try:
            ...
            messagebox.showerror(
                title="Erreur de Syntaxe", message="""Fonction " """ + str(fonction) + """" inconnue""")
            break
...
def onDerive():  # Créé la table de valeur de la dérivée de la courbe
    ...
    if len(choix) == 2:
        messagebox.showerror(title="Erreur de Syntaxe",
        ...
        try:
            ...
            lst1.insert(
                END, "dy/dx  (" + str(ent10.get()) + ")  " + str(select))
            compteur = compteur + 1
...
def onZoom(facteur):
    ...
    if facteur == "init":
        ...
        xmin, xmax, ymin, ymax, pasx, pasy = xmin / facteur, xmax / \
            facteur, ymin / facteur, ymax / facteur, pasx / facteur, pasy / facteur
...
def oncourbe(fichier, couleur, e):
    ...
    try:
        ...
        while a < 699:
            ...
            can.create_line(int(x1), int(y1), int(x2),
                            int(y2), width=e, fill=col)
            a = a + 1
    except:
        flag = 1
...
def onSearch():
    ...
    file = tkFileDialog.askopenfilename(parent=fen, title='Choisissez le fichier .csv à importer:', filetypes=[
                                        ("Tpus les fichiers", "*"), ("Fichiers Excel", "*.csv;*.prn")])
...
def Importer():
    ...
    if flag == 0:
        ...
        file.write(temp)
        file.close()
        messagebox.showinfo(title="Importation", message="""Le classeur " """ +
                            str(fichier) + """" a bien été importé.""")
        root.update()
    elif flag == 1:
        messagebox.showerror(
            title="Importation", message="""Une erreur est survenue lors de l'importation de " """ + str(fichier) + """" .""")
...
def pointeur(event):
    ...
    if cadre == False and selected == False and integer == False:
        chaine.configure(text="Clic détecté en X =" + str(xmin +
                                                          (x) * uxx) + ", Y =" + str(ymin + (700 - y) * uyy))
        croix = [can.create_line(x - 5, y, x + 5, y, fill="red"),
                 can.create_line(x, y - 5, x, y + 5, fill="red")]

    elif cadre == False and selected == True and integer == False:
        chaine.configure(text="Point de la courbe en X =" + str(xmin +
                                                                (x) * uxx) + ", Y =" + str(ymin + (700 - courbe[x]) * uyy))
        croix = [can.create_line(x - 5, courbe[x], x + 5, courbe[x], fill="red"),
                 can.create_line(x, courbe[x] - 5, x, courbe[x] + 5, fill="red")]
        ...
        chaine.configure(text="Calcul d'intégrale: Borne inférieure=" +
                         str(xmin + (x) * uxx) + ", Borne Supérieure")
        croix = [can.create_line(x - 5, courbe[x], x + 5, courbe[x], fill="red"),
                 can.create_line(x, courbe[x] - 5, x, courbe[x] + 5, fill="red")]
        ...
        chaine.configure(text="Calcul d'intégrale: Borne inférieure=" + str(
            xmin + (bornes[0]) * uxx) + ", Borne Supérieure=" + str(xmin + (x) * uxx))
        croix = [can.create_line(x - 5, courbe[x], x + 5, courbe[x], fill="red"),
                 can.create_line(x, courbe[x] - 5, x, courbe[x] + 5, fill="red")]
        ...
        if messagebox.askokcancel('Confirmer', 'La borne inférieure est ' + str(xmin + (bornes[0]) * uxx) + '\nLa borne supérieure est ' + str(xmin + (bornes[1]) * uxx)):
            ...
            if len(choix) == 2:
                ...
                couleur = choix[3]
            chaine.configure(text="Calcul d'intégrale: Aire=" + str(integrale(courbe, bornes[0], bornes[
                             1], couleur)) + " Borne inférieure=" + str(xmin + (bornes[0]) * uxx) + ", Borne Supérieure=" + str(xmin + (x) * uxx))

        else:
            chaine.configure(
                text="Cliquez sur le graphique pour afficher les coordonnées d'un point.")
            onRefresh()
        ...
        if x1 < x2:
            ...
            erreur = True
            messagebox.showerror(
                title="Erreur", message="""Cadre d'épaisseur nulle. \n Les coordonnées d'origine seront restaurées""")
            ent1.delete(0, len(str(xmin)))
            ent2.delete(0, len(str(xmax)))
            ent3.delete(0, len(str(pasx)))
            ent4.delete(0, len(str(ymin)))
...
def onCadre():
    ...
    try:
        ...
        croix = [can.create_line(0, 350, 700, 350, fill="red"), can.create_line(
            350, 0, 350, 700, fill="red")]
    coins = []
...
def onTake():
    ...
    try:
        bornes = [int((eval(inff.get()) * uy) + yo),
                  int((eval(supp.get()) * uy) + yo)]
    ...
    try:
        ...
        messagebox.showerror(
            title="Erreur", message="""Veuillez choisir une courbe""")

    chaine.configure(text="Calcul d'intégrale: Aire=" + str(integrale(courbe, bornes[0], bornes[1], couleur)) + ", Borne inférieure=" + str(
        xmin + (bornes[0]) * uxx) + ", Borne Supérieure=" + str(xmin + (bornes[1]) * uxx))
    pop.destroy()
...
chaine = Label(
    divide, text="Cliquez sur le graphique pour afficher les coordonnées d'un point.")
...
bou15 = Button(root, text="Zoom Initial",
               command=lambda arg="init": onZoom(arg))
