...
def main():
    ...
    for N in range(2, 16):
        ...
        # calculates the conditional number use the numpy API to get the
        # conditional number
        cond_npy = npy.linalg.cond(H, p="fro")
        cond_formal = npy.linalg.norm(npy.dot(abs(H), abs(linalg.inv(H))))
        cond_comp = (npy.linalg.norm(x - xtilde) /
                     npy.linalg.norm(x)) / epsilon
    ...
    for N in range(2, 16):
        ...
        print("    Number of digit of accuracy lost in solving Hx=b, giving by log(cond(H)) is ", aCond[
              N - 2][4])

    print("\n\n\nThe error for the N x N Hilbert matrix becomes O(1) when N =",
          aOlimit[0][0])
