...
"""
    ...
    Construct a set of interpolation nodes with low Lebesgue number on the triangle
    ...
    Tim Warburton, "An Explicit Construction of Interpolation Nodes on the Simplex,"
    Journal of Engineering Mathematics, November 2006, Volume 56, Issue 3, pp 247-262
    ...
    Python implementation by Greg von Winckel on Sun Sep 22 17:56:47 MDT 2013
    
    Modifications from original:
    1) Boundary points have been generalized to Gegenbauer-Gauss-Lobatto nodes 
    2) Now supports up to p=21
    3) Nodes are now ordered as vertices, then edges, then interior points 
       using the counterclockwise indexing convention
...
opt = [[0, 0], [0, 0], [34.5982022, -0.23422571], [-2.01869326e-05, 5.29969190e-01],
       [1.12630113, 0.36465773], [9.70630725e-01,
                                  1.34868622e-04], [1.70233407, 0.61215954],
       ...
       [1.68776479, 0.28637813], [1.75802916, 0.38338828], [
           1.65567624, 5.21832816e-04],
       [1.78616273, -4.44607504e-04], [2.09374663,
                                       0.51318634], [2.01256402, 0.51204339],
...
def nodes(p):
    ...
    def warpfactor(xnodes, xout):
        ...
        for i in range(p + 1):
            d = xnodes[i] - xeq[i]
            d *= reduce(mul, [(xout - xeq[j]) / dxeq[i, j]
                              for j in range(1, i) + range(i + 1, p)], 1)
