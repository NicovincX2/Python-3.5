...
class grid:
    ...
    def norm(self, e):
        ...
        # return
        # numpy.sqrt(self.dx*self.dy*numpy.sum((e[gr.ilo:gr.ihi+1,gr.jlo:gr.jhi+1]**2).flat))
...
def residual(gr, f):
    ...
    r[ib:ie + 1, jb:je + 1] = f[ib:ie + 1, jb:je + 1] - \
        ...
        (gr.phi[ib:ie + 1, jb - 1:je] - 2.0 * gr.phi[ib:ie + 1,
                                                     jb:je + 1] + gr.phi[ib:ie + 1, jb + 1:je + 2]) / gr.dy**2
...
print("Nx, Ny, L2 norm of error (u, v): ", nx, ny,
      numpy.sqrt(gr.dx * gr.dy *
                 numpy.sum((eu[gr.ilo:gr.ihi + 1, gr.jlo:gr.jhi + 1]**2).flat)),
