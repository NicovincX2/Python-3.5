...
def advect(gr, u, S, dt):
    ...
    test[ib:ie + 1] = (u[ib + 1:ie + 2] - u[ib:ie + 1]) * \
        (u[ib:ie + 1] - u[ib - 1:ie])
    ...
    ur[ib:ie + 1] = u[ib:ie + 1] - \
        0.5 * (1.0 + u[ib:ie + 1] * dt / gr.dx) * \
        ldeltau[ib:ie + 1] + 0.5 * dt * S[ib:ie + 1]
...
def lap(gr, u):
    ...
    lapu[ib:ie + 1] = (u[ib - 1:ie] - 2.0 * u[ib:ie + 1] +
                       u[ib + 1:ie + 2]) / gr.dx**2
...
class grid:
    ...
    def initialize(self):
        ...
        u[:] = 1.0
        u[index] += 0.5 * \
            numpy.sin(2.0 * math.pi * (self.x[index] - 0.333) / 0.333)
