...
for cc in range(len(C)):
    for nxx in range(len(nx)):
        ...
        pylab.title("Upwind Adv. Eq.: C=%.2f, %d grid points" %
                    (C[cc], nx[nxx] - 1))

        pylab.plot(g.x[g.ilo:g.ihi + 1], ainit[g.ilo:g.ihi + 1], ls=":")
        pylab.plot(g.x[g.ilo:g.ihi + 1], g.a[g.ilo:g.ihi + 1],
                   label="Explicit Solution")
        pylab.plot(g_im.x[g_im.ilo:g_im.ihi + 1],
                   g_im.a[g_im.ilo:g_im.ihi + 1], label="Implicit Solution")

        pylab.xlabel('Grid', fontsize=10)
        pylab.ylabel('Solution', fontsize=10)

        if (C[cc] == 1):
            pylab.text(0.03, 0.95, "No diffusion for C=1 for Explicit!",
                       color="r", fontsize=9, verticalalignment='top')
            pylab.text(0.03, 0.9, "Solution perfectly advected.",
                       color="r", fontsize=9, verticalalignment='top')

        if (C[cc] <= 0.1):
            pylab.text(0.03, 0.95, "The most accurate solutions",
                       color="r", fontsize=9, verticalalignment='top')
            pylab.text(0.03, 0.9, "still need a small C.",
                       color="r", fontsize=9, verticalalignment='top')

        if (C[cc] > 1):
            pylab.text(0.03, 1.8, "Only implicit solutions",
                       color="r", fontsize=9, verticalalignment='top')
            pylab.text(0.03, 1.3, "converge for any C!", color="r",
