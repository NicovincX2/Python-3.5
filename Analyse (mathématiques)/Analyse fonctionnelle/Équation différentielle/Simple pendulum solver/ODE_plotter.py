...
for i in range(len(CONST_dt)):
    ...
    pylab.plot(histEuler10.t, histEuler10.theta,
               label="Euler's method", color="m", lw=2)
    pylab.plot(histEC10.t, histEC10.theta,
               label="Euler-Cromer's method", color="b", lw=2)
    ...
    pylab.plot(histEuler100.t, histEuler100.theta,
               label="Euler's method", color="m", lw=2)
    pylab.plot(histEC100.t, histEC100.theta,
               label="Euler-Cromer's method", color="b", lw=2)
    pylab.plot(time, ana, 'g*', label="Analytic Approximation")

    pylab.xlabel('Time (s)')
    pylab.ylabel('$\Theta$ (degrees)')
    pylab.title("Simple Pendulum: $\Theta_0$ = $100^o$, $\Delta$t = %s" %
                (str(CONST_dt[i])))
    pylab.grid(True)
    leg = pylab.legend(loc=3, labelspacing=0.005)
    ltext = leg.get_texts()
    ...
    leg.draw_frame(1)
    ax = pylab.gca()
    pylab.savefig("plots/theta_vs_t_for_theta0-100_dt-%s.png" %
                  (str(CONST_dt[i])))
    ...
    pylab.clf()
    pylab.cla()
    pylab.plot(histEuler10.t, histEuler10.energy,
               label="Euler's method", color="r", lw=2)
    pylab.plot(histEC10.t, histEC10.energy,
               label="Euler-Cromer's method", color="b", lw=2)
    pylab.plot(
        time, fun_en, label="Approximation from initial values", color="g", lw=2)
    leg = pylab.legend()
    ltext = leg.get_texts()
    pylab.setp(ltext, fontsize='small')
    leg.draw_frame(0)
    ...
    leg.draw_frame(1)
    ax = pylab.gca()
    pylab.savefig("plots/energy_vs_t_for_theta0-10_dt-%s.png" %
                  (str(CONST_dt[i])))
    ...
    pylab.clf()
    pylab.cla()
    pylab.plot(histEuler100.t, histEuler100.energy,
               label="Euler's method", color="r", lw=2)
    pylab.plot(histEC100.t, histEC100.energy,
               label="Euler-Cromer's method", color="b", lw=2)
    pylab.plot(
        time, fun_en, label="Approximation from initial values", color="g", lw=2)
    leg = pylab.legend()
    ...
    leg.draw_frame(1)
    ax = pylab.gca()
    pylab.savefig("plots/energy_vs_t_for_theta0-100_dt-%s.png" %
                  (str(CONST_dt[i])))
