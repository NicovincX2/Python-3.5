...
# http://codes-sources.commentcamarche.net/source/41405-determination-approximative-de-pi-a-l-aide-de-la-methode-de-monte-carlo
...
# Fonction permettant de démarrer le programme ou bien de le reprendre si
# il a été stoppé
...
# La couleur du point tracé varie donc en fonction de la position du point
# dans le carré
...
# Cette fonction permet le tirage de points de coordonnées aléatoires dans
# le cercle
...
def generateur_nbre_aleat():
    ...
    pts_cercle.configure(
        text="Nombre de points dans le cercle = " + str(pts_dans_cercle))
    pts_tot.configure(
        text="Nombre de point total déjà placés = " + str(pts_total))
    if flag != 0:
...
Button(fra1, text="Quitter", command=fen1.destroy).grid(
