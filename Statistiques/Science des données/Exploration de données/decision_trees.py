...
def __grow_path(tree, coords, input, output, clazz, split_func, max_rm, meta, data_row_indicies, parent_node=None, weight=0):
    ...
    if len(data_row_indicies) <= max_rm:  # add a terminal node
        ...
        tree.node[label]['o'] = k
        if parent_node:
            tree.add_weighted_edges_from([(parent_node, label, weight)])
    ...
    for idx in data_row_indicies:  # loop over data
        ...
        for col in range(input.shape[1]):  # loop over feature space
            ...
            out1 = [output[index] for index in data_row_indicies if input[
                index][col] <= split_value]
            out2 = [output[index]
                    for index in data_row_indicies if input[index][col] > split_value]
    ...
    left_data = [index for index in data_row_indicies if input[
        index][split_j] <= split_value]
    right_data = [index for index in data_row_indicies if input[
        index][split_j] > split_value]
    # level and column for this node
    l, k = [int(x) for x in coords.split(',')]
    left_coords = '{0},{1}'.format(str(l + 1), str(2 * k))
    right_coords = '{0},{1}'.format(str(l + 1), str(2 * k + 1))
    __grow_path(tree, left_coords, input, output, clazz,
                split_func, max_rm, meta, left_data, label, 0)
    __grow_path(tree, right_coords, input, output, clazz,
                split_func, max_rm, meta, right_data, label, 1)
...
def build_tree(input, output, clazz, meta, split_func=cross_entropy, max_rm=5):
    ...
    '''
    tree = nx.DiGraph()
    __grow_path(tree, '0,0', input, output, clazz,
                split_func, max_rm, meta, range(len(output)))
    return tree
...
def decide(tree, input, coords='0,0'):
    ...
    if tree.node[node]['s']:
        ...
        v = input[col]
        if v <= split_val:
            return decide(tree, input, '{0},{1}'.format(str(l + 1), str(2 * k)))
        else:
            return decide(tree, input, '{0},{1}'.format(str(l + 1), str(2 * k + 1)))
    else:
        return tree.node[node]['o']
