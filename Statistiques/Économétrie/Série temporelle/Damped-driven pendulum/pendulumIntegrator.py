...
def intRK4(b, dt, tmax, rhs, theta0, q, omegad):
    ...
    while (t < tmax):
        ...
        thetanew, omeganew = RK4_singlestep(
            b, theta, omega, t, dt, rhs, theta0, q, omegad)
        t += dt
...
def RK4_singlestep(b, theta0tpm, omega0tpm, t, dt, rhs, theta0, q, omegad):
    ...
    thetadot2, omegadot2 = rhs(
        b, theta + 0.5 * dt * thetadot1, omega + 0.5 * dt * omegadot1, t, q, omegad)
    thetadot3, omegadot3 = rhs(
        b, theta + 0.5 * dt * thetadot2, omega + 0.5 * dt * omegadot2, t, q, omegad)
    thetadot4, omegadot4 = rhs(
        b, theta + 0.5 * dt * thetadot3, omega + 0.5 * dt * omegadot3, t, q, omegad)

    # advance
    thetanew = theta + (dt / 6.0) * (thetadot1 + 2.0 *
                                     thetadot2 + 2.0 * thetadot3 + thetadot4)
    omeganew = omega + (dt / 6.0) * (omegadot1 + 2.0 *
