...
def convert_quantum(swarm, rcloud, centre):
    ...
    for part in swarm:
        ...
        # NUVD distribution
        u = abs(random.gauss(0, 1.0 / 3.0))
        part[:] = [(rcloud * x * u / dist) + c for x,
                   c in zip(position, centre)]
...
toolbox.register("particle", generate, creator.Particle, dim=NDIM,
                 pmin=BOUNDS[0], pmax=BOUNDS[1], smin=-
...
def main(verbose=True):
    ...
    RS = (BOUNDS[1] - BOUNDS[0]) / (50**(1.0 / NDIM))
    PMAX = 10
    ...
    while mpb.nevals < 5e5:
        ...
        while sorted_swarm:
            ...
            for s in species:
                dist = math.sqrt(
                    sum((x1 - x2)**2 for x1, x2 in zip(sorted_swarm[0].best, s[0].best)))
                if dist <= RS:
                    found = True
        ...
        swarm = list(itertools.chain(toolbox.swarm(
            n=len(species[-1])), *species[:-1]))
        generation += 1
