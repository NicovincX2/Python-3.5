...
bits_ascii = ''.join(['{:07b}'.format(ord(ch))
                      for ch in "live and let live" if ord(ch) <= 128])
...
corpus = ''.join([ch * (i + 1)
                  for i, ch in enumerate('qwertuioplkjhgfdsazxcvbnm')])
print(corpus)
...
def add_to_node(tree, node, graph):
    left, right = tree
    left_node, right_node = pydot.Node(
        tree_to_str(left)), pydot.Node(tree_to_str(right))
...
flatland_book_clean = flatland_book.replace(chr(65279), '').replace('"', '').replace(
