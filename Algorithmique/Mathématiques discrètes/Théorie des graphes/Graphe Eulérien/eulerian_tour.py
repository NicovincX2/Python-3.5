...
def get_a_tour():
    ...
    # Creating a {node: degree} dictionary for current graph.
    nodes_degree = {}
    for edge in graph:
        a, b = edge[0], edge[1]
            ...
            try:
                if (tour[-1], node) in graph or (node, tour[-1]) in graph:
                    ...
                    try:
                        ...
                        # Updating degree of nodes in the graph, not required
                        # but for the sake of completeness.
                        nodes_degree[tour[-1]] -= 1
                        # Can also be used to check the correctness of program.
                        # In the end all degrees must zero.
        ...
        try:
            while True:
                if node in list(zip(*graph))[0] or node in list(zip(*graph))[1]:
                    ...
                    j = t.index(node)
                    # Joining the two tours.
                    tour = tour[: i] + t[j:-1] + t[:j + 1] + tour[i + 1:]
                    if not graph:       # Found Eulerian Tour
                        return tour     # Returning the Eulerian Tour
                    # Still stuck? Looping back to search for another tour.
                    loop = enumerate(tour[: -1])
                l = loop.__next__()
                i = l[0]
                node = l[1]
        # Oops! seems like the vertices in the current tour cannot connect to
