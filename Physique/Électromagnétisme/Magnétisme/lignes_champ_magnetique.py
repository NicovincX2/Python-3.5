...
class Diagramme(object):
    ...
    def addLine(self, x, y, I=1, points=20):
        ...
        self.maxint = pow(pow(2 * np.pi, 3) * abs(I) *
                          self.size**2 + self.maxint**3, 1 / 3)
    ...
    def addSpire(self, x, y, a, I=1, theta=0, points=20):
        ...
        self.maxint += pow(100 * abs(I) * np.pi * a *
                           a + self.maxint**3, 1 / 3)
    ...
    def addDipole(self, x, y, m, theta=0, points=20):
        ...
        for i in range(points):
            ...
            self.startpoints.append(
                [x0 + s * np.sin(phi), y0 - s * np.cos(phi)])
...
class Spire(MagneticObjects):
    ...
    def B(self, P):
        ...
        # suite des calculs, voir l'article de T.Pré
        # http://www.udppc.asso.fr/bupdoc/textes/fichierjoint/918/0918D119.zip
        k = 4. * abs(r) * a / ((a + abs(r))**2 + z**2)
        Kk = sp.ellipk(k)
        Ek = sp.ellipe(k)
        Br = self.I * (z / r) / np.sqrt((a + abs(r))**2 + z**2) * \
            (-Kk + (a**2 + r**2 + z**2) / ((a - abs(r))**2 + z**2) * Ek)
        Bz = (self.I / np.sqrt((a + abs(r))**2 + z**2)) * \
            (Kk + ((a**2 - r**2 - z**2) / ((a - abs(r))**2 + z**2)) * Ek)
...
class Dipole(MagneticObjects):
    ...
    def B(self, P):
        ...
        y = P[1] - self.y0
        r = np.sqrt(x * x + y * y)
        Bx = (m / (2 * r**3)) * ((-x * np.sin(theta) + y *
                                  np.cos(theta)) * 3 * (x / r**2) + np.sin(theta))
        By = (m / (2 * r**3)) * ((-x * np.sin(theta) + y *
                                  np.cos(theta)) * 3 * (y / r**2) - np.cos(theta))
        return([Bx, By])
