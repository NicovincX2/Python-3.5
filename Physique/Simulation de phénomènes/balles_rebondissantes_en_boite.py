...
singles = [(i, j) for i in range(N)
           for j in range(2)]   # L'ensemble des particules (en 2D)
pairs = [(i, j) for i in range(N)
         for j in range(i + 1, N)]  # L'ensemble des paires
...
def wall_time(pos_a, vel_a, sigma, g):
    ...
    if g == 0:  # Cas d'un axe sans gravité
        ...
        tpossibles = []
        if Delta1 >= 0:
            tpossibles += [1 / g * (-vel_a - Delta1**0.5),
                           1 / g * (-vel_a + Delta1**0.5)]
        if Delta2 >= 0:
            tpossibles += [1 / g * (-vel_a - Delta2**0.5),
                           1 / g * (-vel_a + Delta2**0.5)]
        # print(tpossibles)
        # On ne garde que les positifs
        tpossibles = [t for t in tpossibles if t > 1e-5]
        # print(tpossibles)
        if len(tpossibles) > 0:
...
def compute_next_event(pos, vel):
    ...
    # print('-'*70)
    wall_times = [wall_time(pos[k][l], vel[k][l], sigma, g[l])
                  for k, l in singles]
...
def compute_new_velocities(pos, vel, next_event_arg):
    ...
    if next_event_arg < len(singles):  # Cas d'un choc avec le mur
        ...
        # seule la vitesse sur cet axe est modifiée
...
def snapshot(t, pos, vel, colors, X, Y, arrow_scale=.2):
    ...
    vitesses = np.array([vel[k] for k in range(N) if 0 <= pos[
                        k][1] <= 1 and 0 <= pos[k][0] <= 1])
    positions = np.array([pos[k] for k in range(N) if 0 <= pos[
                         k][1] <= 1 and 0 <= pos[k][0] <= 1])
    # Nombre de particules encore dans le champ
    nb_part = vitesses.shape[0]
    # Intervalle de vitesses regardé
    r = (-2, 2)
    # Dessin de l'histogramme
    pylab.hist(vitesses[:, 0], bins=nb_bins, range=r)
    ...
    # Dispersion (carrée) des vitesses (avec le 2 en plus)
    disp2 = np.sum(vitesses**2) / (2 * N)
    # Normalisation de la gaussienne
    A = N / (sum(np.exp(-(bin_centers)**2 / disp2)))
    # Représentation graphique de la gaussienne en pointillés
    pylab.plot(v, A * np.exp(-v**2 / disp2), '--k', linewidth=3.0)

    # Troisième sous-plot: histogramme de répartition des particules selon
    # l'altitude
    ax3 = pylab.subplot2grid((2, 3), (1, 2), colspan=1, rowspan=1)
    # Intervalle de hauteurs regardé
    r = (0, 1)
    # Dessin de l'histogramme
    pylab.hist(positions[:, 1], bins=nb_bins, range=r)
    pylab.xlim(r)                              # On impose l'abscisse
    pylab.ylim((0, nbmax))                      # et l'ordonnée
    pylab.ylabel("Nombre de particules")       # Légende verticale
    ...
    A = N / (sum(np.exp(g[1] * bin_centers / disp2)))
...
# La dernière particule va être suivie à la loupe
X, Y = [pos[-1][0]], [pos[-1][1]]
next_event, next_event_arg = compute_next_event(
    pos, vel)  # On calcule la première étape
