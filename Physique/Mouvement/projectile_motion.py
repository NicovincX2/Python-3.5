...
class Window(Frame):
    ...
    def init_window(self):
        ...
        self.mbtnPlanet.menu.add_checkbutton(
            label="Mercury", command=self.Mercury)
        ...
        self.mbtnPlanet.menu.add_checkbutton(
            label="Jupiter", command=self.Jupiter)
        self.mbtnPlanet.menu.add_checkbutton(
            label="Saturn", command=self.Saturn)
        self.mbtnPlanet.menu.add_checkbutton(
            label="Uranus", command=self.Uranus)
        self.mbtnPlanet.menu.add_checkbutton(
            label="Neptune", command=self.Neptune)
        ...
        self.btnMotion = Button(
            self, text="Show", command=self.Projectile_Motion, width=10)
        self.btnMotion.grid(row=2, column=1)
        self.btnClear = Button(
            self, text="Clear", command=self.Clear, width=10)
        self.btnClear.grid(row=2, column=2)
        self.btnSphere = Button(
            self, text="360 degree model", command=self.Sphere, width=12)
    ...
    def Projectile_Motion(self):
        ...
        if Planet == "Choose Planet":
            ...
            for t in range(1, 101):
                self.canvas.create_line((Vx * (t - 1)) + 750, -1 * (Vy * (t - 1) + (-0.5 * g * (t - 1)**2)) +
                                        550, (Vx * t) + 750, -1 * (Vy * t + (-0.5 * g * t**2)) + 550, fill=Color, dash=(5, 5))
    ...
    def Sphere(self):
        ...
        if Planet == "Choose Planet":
            ...
            for Angle in range(0, 361):
                ...
                Vy = float(Speed) * math.sin(math.radians(float(Angle)))
                for t in range(1, 101):
                    self.canvas.create_line((Vx * (t - 1)) + 750, -1 * (Vy * (t - 1) + (-0.5 * g * (
                        t - 1)**2)) + 550, (Vx * t) + 750, -1 * (Vy * t + (-0.5 * g * t**2)) + 550, fill=Color, dash=(5, 5))
