...
def evolve(nx, k, C, tmax):
    ...
    while (t < tmax):
        ...
        while (i <= g.ihi):
            ...
            # explicit diffusion
            phinew[i] = g.phi[i] + alpha * \
                (g.phi[i + 1] - 2.0 * g.phi[i] + g.phi[i - 1])
...
while tend <= tmax:
    ...
    pylab.plot(g.x[g.ilo:g.ihi + 1], g.phi[g.ilo:g.ihi + 1],
               color=color, label="$t = %g$ s" % (tend))
    pylab.plot(g.x[g.ilo:g.ihi + 1],
               phi_analytic[g.ilo:g.ihi + 1], color=color, ls="--")
...
pylab.ylabel(r"$\phi$")
pylab.title(
    "Explicit diffusion with varying resolution, C = %3.2f, t = %5.2g" % (C, tmax))
...
pylab.plot(g.x[g.ilo:g.ihi + 1],
           phi_analytic[g.ilo:g.ihi + 1], color="k", ls="--")

pylab.xlim(0.35, 0.65)
pylab.xlabel("$x$")
pylab.ylabel(r"$\phi$")
pylab.title("explicit diffusion, nx = %d, C = %3.2f, t = %5.2g" %
            (nx, C, tmax))
