...
if __name__ == "__main__":
    ...
    pylab.plot(gr.x[gr.ilo:gr.ihi + 1],
               gr.phi[gr.ilo:gr.ihi + 1], color="r", label="C = 0.8")

    C = 2.0
    gr = evolve(nx, k, t0, phi1, phi2, C, tmax)

    pylab.plot(gr.x[gr.ilo:gr.ihi + 1],
               gr.phi[gr.ilo:gr.ihi + 1], color="g", label="C = 2.0")

    C = 10.0
    gr = evolve(nx, k, t0, phi1, phi2, C, tmax)

    pylab.plot(gr.x[gr.ilo:gr.ihi + 1],
               gr.phi[gr.ilo:gr.ihi + 1], color="b", label="C = 10.0")
    ...
    pylab.ylabel(r"$\phi$")
    pylab.title(
        "Backward-difference implicit diffusion, nx = %d, C = %3.2f, t = %5.2g" % (nx, C, tmax))
