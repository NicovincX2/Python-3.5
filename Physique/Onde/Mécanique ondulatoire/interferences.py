...
for t in np.arange(tmin, tmax, dt):  # On boucle sur le temps
    ...
    if shading:
        ...
        rgb = ls.shade(Z1 + Z2, plt.cm.copper)
        plt.imshow(rgb, extent=extent)
    else:
        plt.imshow(Z1 + Z2, interpolation='bilinear',
    ...
    plt.ylim((0, vmax**2))
    plt.plot(x, (source(x, ycut, t, -pos, 0) +
                 source(x, ycut, t, pos, 0, phi))**2)
    ...
    plt.plot((source(xcut, y, t, -pos, 0) +
              source(xcut, y, t, pos, 0, phi))**2, y)
