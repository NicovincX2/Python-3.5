...
def ajoute_fleche(couple, ultra=False, origine=0j, thin=False):
    ...
    * ultra (bool): pour un trait plus gros.'''
    if ultra:
        w, hw = 3, 10           # Veut-on un trait plus épais ?
    elif thin:
        w, hw = 0.1, 0.1        # ou plus fin ?
    else:
        w, hw = 1, 7            # sinon une taille normale
    c = rect(*couple)                # Conversion en complexe
    if abs(c) > max_size:
        raise      # Si on dépasse, c'est la fin !
...
def enonce_et_corrige(nb_ondes=2, num_fichier=1):
    ...
    for i in range(nb_ondes):  # Tirage des ondes
        ...
        # Tirage à pile ou phase pour sinus ou cosinus
        sincos = random.randint(0, 1)
        ...
        if nb_phi == 0:
            ...
            phi_string = '{}{}'.format(signes[signeP], phi)
        else:
            phi_string = '{}{}{}'.format(signes[signeP], nb_phi, phi)
        # Construction de la chaîne de l'énoncé
        enonce = "{}{}(\omega t{})".format(
            Astring, sin_cos[sincos], phi_string)
    ...
    try:    # Cas où cela dépasse du cadre
        corrige = fabrique_corrige_fresnel(
            ondes_corrige, ondes_enonce, num_fichier)
...
def fabrique_corrige_fresnel(liste_des_couples, ondes_enonces, num_fichier):
    ...
    fichier = 'PNG/{}ondes_corrige_{:03d}'.format(
        len(ondes_enonces), num_fichier)
    plt.savefig(fichier)
    return fichier
