...
plt.plot([deb_fleche, x_fin, deb_fleche], [
         altitude_fleche, 0, -altitude_fleche], '-k')  # flèche
...
plt.plot([x_O - lx_fl, x_O, x_O + lx_fl], [0.95 * y_fin -
                                           ly_fl, 0.95 * y_fin, 0.95 * y_fin - ly_fl], '-k', lw=2)
plt.plot([x_O - lx_fl, x_O, x_O + lx_fl], [0.95 * y_deb +
                                           ly_fl, 0.95 * y_deb, 0.95 * y_deb + ly_fl], '-k', lw=2)
# foyers
...
lx_fl = 0.025 * delta_x
ly_fl = sg * 0.025 * delta_y
plt.plot([x_A_p - lx_fl, x_A_p, x_A_p + lx_fl],
         [A_pB_p - ly_fl, A_pB_p, A_pB_p - ly_fl], style)

# premier rayon facile : par le centre et par l'objet
# thalès AB/(xO-xA) = y/(x_O-xdeb)
plt.plot([x_deb, x_fin], [(x_O - x_deb) / (x_O - x_A)
                          * AB, (x_O - x_fin) / (x_O - x_A) * AB], '-r')
...
if (abs(x_A - x_O + f_p) > 2**(-8) * delta_x):
    plt.plot([x_deb, x_O, x_fin], [(x_deb - x_O + f_p) /
                                   (f_p) * A_pB_p, A_pB_p, A_pB_p], '-g')
...
print(r"egin{pspicture*}(", x_deb - 0.1, ",", y_deb -
      0.1, ")(", x_fin + 0.1, ",", y_fin + 0.1, ")")
# Axe optique
print(r"% axe optique")
...
if (f_p < 0):
    ...
    arrow = '<->'
print(r"%Lentille")
print(r"\psline[linewidth = 2pt,arrowscale = 2]{", arrow,
      "}(", x_O, ",", y_deb, ")(", x_O, ",", y_fin, ")")
...
print(r"\psline[linewidth = 1.5pt" + style +
      "{->}(", x_A, ",0)(", x_A, ",", AB, ")")
if AB > 0:
    sg = 1
else:
...
print(r"\psline[linecolor = red,ArrowInside = ->,arrowscale = 2](", x_deb, ",", (x_O - x_deb) /
      (x_O - x_A) * AB, ")(", x_O, ",0)(", x_fin, ",", (x_O - x_fin) / (x_O - x_A) * AB, ")")
# 2e rayon facile : parallèle à l'axe optique puis sort en passant par F'
