...
Programme similaire à lois_de_descartes.py si ce n'est que l'on part de la 
...
from math import *
# NB: comme turtle est importé en second, radians() vient de turtle et non
# de math
from turtle import *
...
for i in range(1, 91):   # i est l'angle incident (donc dans le plexiglas)
    if i % 10 == 9:
        # Changement de couleur tous les 10 degrés
        color(COLORS[(i // 10 + 1) % 2])
    i = pi * i / 180        # Conversion en radians
    # Si on peut prendre l'arcsinus
    try:
        r = asin(n * sin(i))  # On le fait
    ...
    if fin:
        break       # Si c'est fini, on sort de la boucle
