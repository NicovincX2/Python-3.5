...
# Question: given a range a,b, find the total number of perfect square
...
def find_perfect_square_count(a, b, verbose=False):
    ...
    if a < 0 or b < 0:
        ...
        # i axis, in this case we only need the min(a,b) otherwise we will
        # double count
        if a < 0 and b < 0:
            return np.sum([math.ceil(cmath.sqrt(complex(min(a, b), 0)).imag)]) - 1

    if a >= b:
        count = 2 * (math.ceil(math.sqrt(abs(a))) -
                     math.ceil(math.sqrt(abs(b))))
        if count > 0:
            return count - 2
        else:
            return 0
    ...
    if b >= a:
        ...
        count = 2 * (math.ceil(math.sqrt(abs(b))) -
                     math.ceil(math.sqrt(abs(a))))
        if count > 0:
            return count - 2
        else:
            return 0
...
number_of_tests = 5
value = 1e4
for (a, b) in zip([random.randint(-value, value) for x in arange(number_of_tests)], [random.randint(-value, value) for x in arange(number_of_tests)]):
