...
class FenetrePrincipale(QWidget):
    ...
    def __init__(self):
        ...
        self.doubleSpinBox_N2.setButtonSymbols(
        ...
        self.doubleSpinBox_H2.setButtonSymbols(
            QtGui.QAbstractSpinBox.NoButtons)
        ...
        self.glayout_bilan.addWidget(
            self.label_N2, 0, 0, QtCore.Qt.AlignHCenter)
        self.glayout_bilan.addWidget(
            self.doubleSpinBox_N2, 1, 0, QtCore.Qt.AlignHCenter)
        self.glayout_bilan.addWidget(QLabel("+"), 0, 1, QtCore.Qt.AlignHCenter)
        self.glayout_bilan.addWidget(
            self.label_H2, 0, 2, QtCore.Qt.AlignHCenter)
        self.glayout_bilan.addWidget(
            self.doubleSpinBox_H2, 1, 2, QtCore.Qt.AlignHCenter)
        self.glayout_bilan.addWidget(
            QLabel("   =   2 NH<sub>3<\sub>"), 0, 3, QtCore.Qt.AlignHCenter)
        ...
        self.glayout_P_T.addWidget(
            QLabel("Pression totale :"), 0, 0, QtCore.Qt.AlignLeft)
        self.glayout_P_T.addWidget(
            self.doubleSpinBox_Ptot, 0, 1, QtCore.Qt.AlignRight)
        self.glayout_P_T.addWidget(
            QLabel("Temperature :"), 1, 0, QtCore.Qt.AlignLeft)
        self.glayout_P_T.addWidget(
            self.doubleSpinBox_T, 1, 1, QtCore.Qt.AlignRight)
        ...
        self.connect(self.radio_N2, SIGNAL(
            "toggled(bool)"), self.slot_radio_N2)
        self.connect(self.radio_H2, SIGNAL(
            "toggled(bool)"), self.slot_radio_H2)
        ...
        self.doubleSpinBox_variation_1.setDecimals(
            self.doubleSpinBox_N2.decimals())
        self.doubleSpinBox_variation_1.setRange(
            self.doubleSpinBox_N2.minimum(), self.doubleSpinBox_N2.maximum())
        self.doubleSpinBox_variation_1.setValue(
            self.doubleSpinBox_N2.minimum())
        ...
        self.doubleSpinBox_variation_pas.setRange(
            self.doubleSpinBox_N2.minimum(), self.doubleSpinBox_N2.maximum() / 2)
        ...
        self.vlayout_parametres.addWidget(
            QLabel("<b>Optimisation des parametres :<\b>"))
    ...
    def slot_radio_H2(self):
        ...
        self.doubleSpinBox_T.setEnabled(True)
        self.doubleSpinBox_variation_1.setDecimals(
            self.doubleSpinBox_H2.decimals())
        self.doubleSpinBox_variation_1.setRange(
            self.doubleSpinBox_H2.minimum(), self.doubleSpinBox_H2.maximum())
        self.doubleSpinBox_variation_1.setValue(
            self.doubleSpinBox_H2.minimum())
        self.doubleSpinBox_variation_2.setDecimals(
            self.doubleSpinBox_H2.decimals())
        self.doubleSpinBox_variation_2.setRange(
            self.doubleSpinBox_H2.minimum(), self.doubleSpinBox_H2.maximum())
        self.doubleSpinBox_variation_2.setValue(
            self.doubleSpinBox_H2.maximum())
        self.doubleSpinBox_variation_pas.setDecimals(
            self.doubleSpinBox_H2.decimals())
        self.doubleSpinBox_variation_pas.setRange(
            self.doubleSpinBox_H2.minimum(), self.doubleSpinBox_H2.maximum() / 2)
        self.doubleSpinBox_variation_pas.setValue(
            self.doubleSpinBox_H2.minimum())
    ...
    def slot_calculer(self):
        ...
        if self.radio_T.isChecked() == True:
            xi_max = np.minimum(self.doubleSpinBox_N2.value(),
                                self.doubleSpinBox_H2.value() / 3.0)
            ...
            while i - self.doubleSpinBox_variation_pas.value() < self.doubleSpinBox_variation_2.value():
                ...
                while j < precision:
                    ...
                    if rdt < 0.999999:
                        xi = rdt * xi_max
                        qr = (2 * xi)**2 * (self.doubleSpinBox_N2.value() + self.doubleSpinBox_H2.value() - 2 * xi)**2 / (
                            (self.doubleSpinBox_N2.value() - xi) * (self.doubleSpinBox_H2.value() - 3 * xi)**3 * (self.doubleSpinBox_Ptot.value())**2)
                ...
                x_NH3 = (2 * rdt * xi_max) / (self.doubleSpinBox_N2.value() +
                                              self.doubleSpinBox_H2.value() - 2 * rdt * xi_max)
            ...
            while i - self.doubleSpinBox_variation_pas.value() < self.doubleSpinBox_variation_2.value():
                ...
                while j < precision:
                    ...
                    if rdt < 0.999999:
                        xi = rdt * xi_max
                        qr = (2 * xi)**2 * (self.doubleSpinBox_N2.value() + self.doubleSpinBox_H2.value() - 2 * xi)**2 / (
                            (self.doubleSpinBox_N2.value() - xi) * (self.doubleSpinBox_H2.value() - 3 * xi)**3 * (i**2))
                        if qr > k0:
                            rdt -= pas
                            pas /= 10
                            j += 1
        ...
        if self.radio_N2.isChecked() == True:
            ...
            while i - self.doubleSpinBox_variation_pas.value() < self.doubleSpinBox_variation_2.value():
                ...
                while j < precision:
                    ...
                    if rdt < 0.999999:
                        xi = rdt * xi_max
                        qr = (2 * xi)**2 * (i + self.doubleSpinBox_H2.value() - 2 * xi)**2 / ((i - xi) * (
                            self.doubleSpinBox_H2.value() - 3 * xi)**3 * (self.doubleSpinBox_Ptot.value()**2))
                        if qr > k0:
                            rdt -= pas
                            pas /= 10
                            j += 1
                ...
                x_NH3 = (2 * rdt * xi_max) / \
                    (i + self.doubleSpinBox_H2.value() - 2 * rdt * xi_max)
                self.texte_resultat += str(np.round(i, 2)) + "       " + str(
                    np.round(rdt, 4)) + "       " + str(np.round(x_NH3, 4)) + "\n"
                self.qmc.tableau_rdt.append(rdt)
                self.qmc.tableau_xNH3.append(x_NH3)
                #--incrementation de i--
        ...
        if self.radio_H2.isChecked() == True:
            ...
            while i - self.doubleSpinBox_variation_pas.value() < self.doubleSpinBox_variation_2.value():
                ...
                while j < precision:
                    ...
                    if rdt < 0.999999:
                        xi = rdt * xi_max
                        qr = (2 * xi)**2 * (i + self.doubleSpinBox_N2.value() - 2 * xi)**2 / (
                            (self.doubleSpinBox_N2.value() - xi) * (i - 3 * xi)**3 * (self.doubleSpinBox_Ptot.value()**2))
                        if qr > k0:
                            rdt -= pas
                            pas /= 10
                            j += 1
                ...
                x_NH3 = (2 * rdt * xi_max) / \
                    (i + self.doubleSpinBox_N2.value() - 2 * rdt * xi_max)
                self.texte_resultat += str(np.round(i, 2)) + "       " + str(
                    np.round(rdt, 4)) + "       " + str(np.round(x_NH3, 4)) + "\n"
                self.qmc.tableau_rdt.append(rdt)
                self.qmc.tableau_xNH3.append(x_NH3)
