...
class MplCanvas(FigureCanvas):
    ...
    def __init__(self, parent=None):
        ...
        self.l_tableau_rdt, = self.axes.plot(
            self.tableau_parametre, self.tableau_rdt, label="Rendement")
        self.l_tableau_xNH3, = self.axes.plot(
            self.tableau_parametre, self.tableau_xNH3, label="x(NH3)")
        self.axes.legend()
...
class FenetrePrincipale(QWidget):
    ...
    def __init__(self):
        ...
        self.doubleSpinBox_rapportH2N2.setButtonSymbols(
            QtGui.QAbstractSpinBox.NoButtons)
        ...
        self.glayout_P_T.addWidget(
            QLabel("<b>Parametres initiaux :<\b>"), 0, 0, 1, -1)
        self.glayout_P_T.addWidget(
            QLabel("n(N2) + n(H2) :"), 1, 0, QtCore.Qt.AlignLeft)
        self.glayout_P_T.addWidget(
        ...
        self.glayout_P_T.addWidget(
            QLabel("n(H2) / n(N2) :"), 2, 0, QtCore.Qt.AlignLeft)
        self.glayout_P_T.addWidget(
            self.doubleSpinBox_rapportH2N2, 2, 1, QtCore.Qt.AlignRight)
        self.glayout_P_T.addWidget(
        ...
        self.connect(self.radio_rapportH2N2, SIGNAL(
            "toggled(bool)"), self.slot_radio_rapportH2N2)
        ...
        self.doubleSpinBox_variation_1.setDecimals(
            self.doubleSpinBox_rapportH2N2.decimals())
        self.doubleSpinBox_variation_1.setRange(
            self.doubleSpinBox_rapportH2N2.minimum(), self.doubleSpinBox_rapportH2N2.maximum())
        self.doubleSpinBox_variation_1.setValue(
            self.doubleSpinBox_rapportH2N2.minimum())
        ...
        self.doubleSpinBox_variation_2.setValue(
            self.doubleSpinBox_rapportH2N2.maximum())
        ...
        self.doubleSpinBox_variation_pas.setDecimals(
            self.doubleSpinBox_rapportH2N2.decimals())
        self.doubleSpinBox_variation_pas.setRange(self.doubleSpinBox_rapportH2N2.minimum(
        ), self.doubleSpinBox_rapportH2N2.maximum() / 2)
        ...
        self.vlayout_parametres.addWidget(
            QLabel("N2 (g) + 3 H2 (g) = 2 NH3 (g)"), 0, QtCore.Qt.AlignHCenter)
        self.vlayout_parametres.addLayout(self.glayout_P_T)
        self.vlayout_parametres.addSpacing(20)
        self.vlayout_parametres.addWidget(
            QLabel("<b>Variation d'un des parametres initiaux :<\b>"))
    ...
    def slot_radio_ntot(self):
        ...
        self.qmc.axes.set_xlabel(
            "Quantite de matiere totale initiale n(N2) + n(H2) (en mol)")
    ...
    def slot_calculer(self):
        ...
        if self.radio_T.isChecked() == True:
            n_N2_init = self.doubleSpinBox_n_init.value(
            ) / (1 + self.doubleSpinBox_rapportH2N2.value())
            n_H2_init = self.doubleSpinBox_n_init.value() * self.doubleSpinBox_rapportH2N2.value() / \
                (1 + self.doubleSpinBox_rapportH2N2.value())
            ...
            while i - self.doubleSpinBox_variation_pas.value() < self.doubleSpinBox_variation_2.value():
                ...
                while j < precision:
                    ...
                    if rdt < 0.999999:
                        xi = rdt * xi_max
                        qr = (2 * xi)**2 * (n_N2_init + n_H2_init - 2 * xi)**2 / ((n_N2_init - xi)
                                                                                  * (n_H2_init - 3 * xi)**3 * (self.doubleSpinBox_Ptot.value())**2)
                        if qr > k0:
                            rdt -= pas
                            pas /= 10
                            j += 1
                ...
                x_NH3 = (2 * rdt * xi_max) / \
                    (n_N2_init + n_H2_init - 2 * rdt * xi_max)
                self.texte_resultat += str(np.round(i, 0)) + "       " + str(
                    np.round(rdt, 4)) + "       " + str(np.round(x_NH3, 4)) + "\n"
        ...
        if self.radio_Ptot.isChecked() == True:
            ...
            while i - self.doubleSpinBox_variation_pas.value() < self.doubleSpinBox_variation_2.value():
                ...
                while j < precision:
                    ...
                    if rdt < 0.999999:
                        xi = rdt * xi_max
                        qr = (2 * xi)**2 * (n_N2_init + n_H2_init - 2 * xi)**2 / \
                            ((n_N2_init - xi) * (n_H2_init - 3 * xi)**3 * (i**2))
                        if qr > k0:
                            rdt -= pas
                            pas /= 10
                            j += 1
        ...
        if self.radio_ntot.isChecked() == True:
            ...
            while i - self.doubleSpinBox_variation_pas.value() < self.doubleSpinBox_variation_2.value():
                ...
                n_H2_init = i * self.doubleSpinBox_rapportH2N2.value() / \
                    (1 + self.doubleSpinBox_rapportH2N2.value())
                xi_max = np.minimum(n_N2_init, n_H2_init / 3.0)
                self.qmc.tableau_parametre.append(i)
                #--Recherche de la valeur de xi telle que qr = k0--
                ...
                x_NH3 = (2 * rdt * xi_max) / \
                    (n_N2_init + n_H2_init - 2 * rdt * xi_max)
                self.texte_resultat += str(np.round(n_N2_init, 3)) + "       " + str(np.round(
                    n_H2_init, 3)) + "       " + str(np.round(rdt, 4)) + "       " + str(np.round(x_NH3, 4)) + "\n"
                self.qmc.tableau_rdt.append(rdt)
                self.qmc.tableau_xNH3.append(x_NH3)
                #--incrementation de i--
                i += self.doubleSpinBox_variation_pas.value()
        ...
        self.qmc.l_tableau_xNH3.set_data(
            self.qmc.tableau_parametre, self.qmc.tableau_xNH3)
