...
Hmin = 0.9 * CP.PropsSI('H', 'P', Pmin *
                        CONVERSION[unitP], 'Q', 0, fluide) / CONVERSION[unitH]
# et l'enthalpie à droite un peu au-dessus de celle du point triple côté gaz
Hmax = 1.9 * CP.PropsSI('H', 'P', Pmin *
                        CONVERSION[unitP], 'Q', 1, fluide) / CONVERSION[unitH]

# Données pour les isothermes (on veut une représentation sympa en celsius
# en pratique)
dT = 20                                 # Incrément de températures
# Valeur de la température au point triple
Ttriple = CP.PropsSI(fluide, 'Ttriple')
...
Tmin = kelvin(20)
# et on dépasse un peu le point critique
val_T = np.arange(Tmin, 1.5 * Tcrit, dT)
# Sélection des T à afficher (mettre None pour toutes)
T_to_show = list(range(2, len(val_T), 2))
...
ds = 0.5e3
striple_x0 = CP.PropsSI('S', 'Q', 0, 'T', Ttriple,
                        fluide)  # Entropie triple à gauche
striple_x1 = CP.PropsSI('S', 'Q', 1, 'T', Ttriple,
                        fluide)  # Entropie triple à droite
...
vcrit = 1 / CP.PropsSI(fluide, 'rhocrit')
exp_min = int(np.floor(np.log10(vcrit))) + \
    1             # Puissance de 10 proche
vtriple_x1 = 1 / CP.PropsSI('D', 'Q', 1, 'T', Ttriple,
                            fluide)  # Point triple à droite
exp_max = int(np.ceil(np.log10(vtriple_x1))) - \
    1         # Puissance de 10 proche
...
def place_label(x, y, label, indice=None, cotan=False, color='k'):
    ...
    if indice:
        N = indice    # sauf si l'utilisateur impose la valeur
    ...
    if Plogscale:
        ...
        a = (np.log10(y[N + 1]) - np.log10(y[N - 1])) / \
            (x[N + 1] - x[N - 1]) * Xsize / Ysize
    else:
        Ysize = yf - yi
    ...
    t.set_bbox(dict(boxstyle="round", facecolor='w',
...
def fait_isolignes(type, valeurs, position=None, nb_points=1000, to_show=None, round_nb=0):
    ...
    if Plogscale:
        val_P0 = np.logspace(np.log10(Pmin), np.log10(Pmax), nb_points)
    ...
    for val, i in zip(valeurs, range(len(valeurs))):
        if type == 'V':  # Cas particulier des volumes massiques: échantillonnage
            val_P = CP.PropsSI('P', 'T', val_T, 'D', 1 /
                               val, fluide)  # en température
            val_H = CP.PropsSI('H', 'T', val_T, 'D', 1 /
...
def affiche_serie_points(couples):
    for P, T in couples:
        T = kelvin(T)
        H = CP.PropsSI('H', 'T', T, 'P', P *
                       CONVERSION[unitP], fluide) / CONVERSION[unitH]
...
if iso_T:
    fait_isolignes('T', val_T, position=0.9, to_show=T_to_show)
if iso_s:
    fait_isolignes('S', val_s, position=0.5, to_show=s_to_show, round_nb=3)
if iso_v:
    fait_isolignes('V', val_v, position=0.25, to_show=v_to_show, round_nb=3)
if iso_x:
    fait_isolignes('Q', val_x, position=0.1, to_show=x_to_show, round_nb=2)
...
def place_point(H, P, S, T):
    ...
    t = int(P / CONVERSION[unitP]), int(H /
                                        1e3), round(S / 1e3, 2), int(round(celsius(T))), 0
    plt.plot(H / CONVERSION[unitH], P / CONVERSION[unitP], 'ko', markersize=8,
             label='$P={}$ bars, $h={}$ kJ/kg, $s={}$ kJ/K/kg et $T={}^{{\circ}}$C'.format(*t))
    # plt.plot(H/CONVERSION[unitH],P/CONVERSION[unitP],'ko',markersize=8,
    # label = '$s={}$ kJ/K/kg et $T={}^{{\circ}}$C'.format(round(S/1e3,2),int(
...
if mettre_Fessenheim:
    ...
    place_point(H, P, S, T)
    # Le sécheur surchauffeur le remet jusqu'à 267°C (la pression baisse un
    # peu)
    P = 9.4e5
    T = kelvin(267)
...
Fessenheim = [(67.5, 220),
              ...
              # Sortie sécheur surchauffeur, arrivée turbines "basses pression"
              (9.4, 267),
