...
dh = 100e3
htriple_x0 = CP.PropsSI('H', 'Q', 0, 'T', Ttriple,
                        fluide)  # Entropie triple à gauche
htriple_x1 = CP.PropsSI('H', 'Q', 1, 'T', Ttriple,
...
# Quelques constantes
UNITS = {'T': 'K', 'Q': '', 'S': 'kJ/K/kg',
         'V': 'm$^3$/kg', 'P': 'bar', 'H': 'kJ/kg'}
LABEL = {'T': 'T', 'Q': 'x', 'S': 's',
         'V': 'v',       'P': 'P',  'H': 'h'}
COLOR_MAP = {'T': 'Darkred',
             'P': 'DarkCyan',
...
def place_label(x, y, label, indice=None, cotan=False, color='k'):
    ...
    if cotan and rot < 0:
        rot = 180 + rot             # Si on dépasse la     verticale
    if cotan:
...
if iso_P:
    fait_isolignes('P', val_P, position=0.8, to_show=P_to_show, round_nb=3)
if iso_h:
    fait_isolignes('H', val_h, position=0.8, to_show=h_to_show, round_nb=3)
if iso_v:
