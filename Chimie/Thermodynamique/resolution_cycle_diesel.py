...
# Récupération pression (faut ruser)
P[2] = find_P_from_v_s(v[2], s[2], P[1], P[3])
# Température correspondante
T[2] = CP.PropsSI('T', 'P', P[2], 'S', s[2], gaz)
v[3] = v[2]                                    # Isochore
# dont on connaît la pression finale
T[3] = CP.PropsSI('T', 'P', P[3], 'D', 1 / v[3], gaz)
v[4] = 1 / CP.PropsSI('D', 'P', P[4], 'T', T[4], gaz)  # Isobare à T connue
# et calcul de l'entropie correspondante
s[4] = CP.PropsSI('S', 'P', P[4], 'T', T[4], gaz)
s[5] = s[4]                                    # Isentropique
v[5] = v[1]                                    # Dernière isochore
P[5] = find_P_from_v_s(v[5], s[5], P[1], P[3])    # Ruse sioux pour la pression
# et obtention de la T correspondante
T[5] = CP.PropsSI('T', 'P', P[5], 'D', 1 / v[5], gaz)
...
v12 = 1 / CP.PropsSI('D', 'P', P12, 'S', s[1], gaz)
...
def infos_point(nb, P, T, v):
    print('Infos pour le point {0}: T={1} K, v={3} m^3/kg, P={2} bar'.format(
        nb, round(T, 1), round(P / 1e5, 1), round(v, 4)))
...
for i in range(len(L_v)):
    plt.plot(L_v[i], L_P[i] / 1e5,
             label='{}$\\to${}, reel'.format(i + 1, (i + 1) % 5 + 1))
...
plt.savefig('T6_resolution_cycle_diesel.png')
