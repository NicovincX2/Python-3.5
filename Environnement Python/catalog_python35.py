...
def entry_to_line(entry, affichage):
    if affichage.choix == 1:
        line = "{} {} {}".format(entry.name.ljust(
            75), entry.drive_name.ljust(15), entry.dir_minus_drive)
        return line
    elif affichage.choix == 2:
        line = "{} {} {}".format(entry.drive_name.ljust(
            15), entry.dir_minus_drive.ljust(160), entry.name)
        return line
...
def write_catalog_file_entries(catalog_path, entries, affichage):
    ...
    if affichage.choix == 1:
        text = "{} {} {}".format(nom.ljust(75), drive.ljust(
            15), dir) + "\n" + "\n".join(lines)
        write_file(catalog_path, text, False)
    elif affichage.choix == 2:
        text = "{} {} {}".format(drive.ljust(15), dir.ljust(
            160), nom) + "\n" + "\n".join(lines)
        write_file(catalog_path, text, False)
...
def sample():
    ...
    affichage = Affichage(int(input('Affichage du catalogue: [1] --> Programmes, Disques, Répertoires \n\t\t\t[2] --> Disques, Répertoires, Programmes \n ')), input(
        'Afficher les fichiers README.md: [Yes/No]'))
    ...
    write_drive_catalog_file("C:\Python35", "Python35",
                             r"C:\Python35\Environnement Python\Python3_catalog.txt", affichage)
    # write_master_catalog_file([r"C:\Python35\Python35drive.txt"], r"C:\Python35\Python35master.txt", affichage)
    entries = read_catalog_file_entries(
        r"C:\Python35\Environnement Python\Python3_catalog.txt")
    for entry in entries:
