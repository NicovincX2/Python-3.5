...
def circle_in_three_circles_with_one_inside_out(circ1, circ2, circ3, other_one=False):
    ...
    errpos = abs(np.linalg.norm(np.array(circ2.cen) - np.array([centrepos.real, centrepos.imag])) - (radius + circ2.rad)) \
        + abs(np.linalg.norm(np.array(circ3.cen) -
                             np.array([centrepos.real, centrepos.imag])) - (radius + circ3.rad))
    errneg = abs(np.linalg.norm(np.array(circ2.cen) - np.array([centreneg.real, centreneg.imag])) - (radius + circ2.rad)) \
        + abs(np.linalg.norm(np.array(circ3.cen) -
                             np.array([centreneg.real, centreneg.imag])) - (radius + circ3.rad))
...
def pack_circles_in_circle(centre1, centre2, radius_ratio, radius_limit):
    ...
    new_circle1 = circle_in_three_circles_with_one_inside_out(
        circle_list[0], circle_list[1], circle_list[2])
    ...
    todo_list = deque([(0, 1, 3), (0, 2, 3), (1, 2, 3),
                       (0, 1, 4), (0, 2, 4), (1, 2, 4)])
    counter = 4
    ...
    while todo_list:
        ...
        if parents[0] == 0:
            ...
            new_circle = circle_in_three_circles_with_one_inside_out(
...
def draw_circle_fractal(ax, centre1, centre2, radius_ratio, radius_limit,
    ...
    """
    circle_list = pack_circles_in_circle(
        centre1, centre2, radius_ratio, radius_limit)
...
draw_circle_fractal(ax, centre1, centre2, radius_ratio,
                    radius_limit, linewidth=1)
