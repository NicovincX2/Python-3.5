...
# f : facteur d'échelle lié à la résolution de l'écran du PC : on peut
...
# Les trois points G, I , N  sont alignés, avec la relation : GN =2*GI (on
# a aussi HN = 2*OI)
...
def pop_up(event=None):
    ...
    menu.add_command(label='Présentation du script',
                     underline=0, command=rahxephon)
    menu.add_separator()
    menu.add_command(label='Triangle', underline=0, command=animation_triangle)
    menu.add_separator()
    menu.add_command(label='Centre de gravité',
                     underline=0, command=centre_de_gravite)
    menu.add_command(label='Orthocentre', underline=0, command=ortho_centre)
    menu.add_command(label='Cercle circonscrit',
                     underline=0, command=cercle_circonscrit)
    menu.add_command(label='Cercle et droite d\' Euler',
                     underline=0, command=Euler)
    menu.add_command(label='Cercle inscrit',
                     underline=0, command=cercle_inscrit)
    menu.add_command(label='Points de Gergonne et de Nagel',
                     underline=0, command=Gergonne_Nagel)
...
def naruto(event=None):
    ...
    dxA, dyA, dxB, dyB, dxC, dyC = 0.2 * f, 0.2 * \
        f, 0.2 * f, 0.2 * f, 0.2 * f, -0.2 * f
...
def animer():
    ...
    # coordonnées des vecteurs vitesses des sommets A, B, C
    global dxA, dyA, dxB, dyB, dxC, dyC
    global a, b, c, p  # longueur des côtés BC,CA,AB et demi périmètre
    global A, B, C  # angles aux sommets A,B,C
    # coordonnées des milieux des côtés BC, CA, AB
    global xmA, ymA, xmB, ymB, xmC, ymC
    global xG, yG  # coordonnées du centre de gravité (G)
    # coordonnées des pieds des hauteurs issues des trois sommets A, B, C
    global xhA, yhA, xhB, yhB, xhC, yhC
    global xH, yH  # coordonnées de l'orthocentre (H)
    # coordonnées du centre (O) et rayon (RO) du cercle circonscrit
    global xO, yO, RO
    # coordonnées du centre (E) et rayon (RE) du cercle d'Euler
    global xE, yE, RE
    # coordonnées des milieux des segments joignant les trois sommets à
    # l'orthocentre
    global xaA, yaA, xbB, ybB, xcC, ycC
    # coordonnées des points de Gergonne et de Nagel
    global xGergonne, yGergonne, xNagel, yNagel
    # coordonnées du centre (I) et rayon (RI) du cercle inscrit
    global xI, yI, RI
    # coordonnées des points de contact entre le cercle inscrit et les côtés
    # du triangle
    global xiA, yiA, xiB, yiB, xiC, yiC
    # coordonnées des points de contact entre les cercles exinscrits et les
    # côtés du triangle
    global xeA, yeA, xeB, yeB, xeC, yeC
    ...
    if flag == 1:
        can.delete(ALL)
        # animation
        xA, yA, xB, yB, xC, yC = xA + k * dxA, yA + k * dyA, xB + \
            k * dxB, yB + k * dyB, xC + k * dxC, yC + k * dyC

        # résolution du triangle
        a, b, c = hypot(xC - xB, yC - yB), hypot(xA - xC,
                                                 yA - yC), hypot(xA - xB, yA - yB)
        p = (a + b + c) / 2  # demi périmètre (p)
        # rayon (RI) du cercle inscrit
        RI = sqrt((p - a) * (p - b) * (p - c) / p)
        if p == a:
            A = 0
        if p == b:
            B = 0
        ...
        if p == a or p == b or p == c:
            ...
            A, B, C = 2 * atan(RI / (p - a)), 2 * atan(RI / (p - b)
                                                       ), 2 * atan(RI / (p - c))  # angles aux sommets
            RO = a * b * c / RI / p / 4  # rayon (RO) du cercle circonscrit
        RE = RO / 2  # rayon (RE) du cercle d'Euler

        # coordonnées des milieux des côtés BC, CA, AB et du centre de gravité
        # G ( isobarycentre : A[1], B[1], C[1] )
        barycentre(0, 1, 1)
        xmA, ymA = x, y
        barycentre(1, 0, 1)
        xmB, ymB = x, y
        ...
        # coordonnées des pieds des hauteurs issues des trois sommets A, B, C
        # et de l'orthocentre H : barycentre de A[tan(A)], B[tan(B)], C[tan(C)]
        barycentre(0, tan(B), tan(C))
        xhA, yhA = x, y
        barycentre(tan(A), 0, tan(C))
        xhB, yhB = x, y
        barycentre(tan(A), tan(B), 0)
        xhC, yhC = x, y
        barycentre(tan(A), tan(B), tan(C))
        xH, yH = x, y

        # coordonnées du centre (O)  du cercle circonscrit  : barycentre de
        # A[sin(2*A)], B[sin(2*B)], C[sin(2*C)]
        barycentre(sin(2 * A), sin(2 * B), sin(2 * C))
        xO, yO = x, y
        ...
        # coordonnées du point de Gergonne : barycentre de A[(p-b)*(p-c)],
        # B[(p-c)*(p-a)], C[(p-a)*(p-b)]
        barycentre((p - b) * (p - c), (p - c) * (p - a), (p - a) * (p - b))
        xGergonne, yGergonne = x, y

        # coordonnées du point de Nagel : barycentre de A[p-a], B[p-b], C[p-c]
        barycentre(p - a, p - b, p - c)
        xNagel, yNagel = x, y

        # coordonnées du centre (I) du cercle inscrit : barycentre de A[a],
        # B[b], C[c]
        barycentre(a, b, c)
        xI, yI = x, y

        # coordonnées des points de contact entre le cercle inscrit et les
        # côtés du triangle
        if p == a:
            xiA, yiA = 0, 0
        else:
            barycentre(0, (p - c) * (p - a), (p - a) * (p - b))
            xiA, yiA = x, y
        if p == b:
            xiB, yiB = 0, 0
        else:
            barycentre((p - b) * (p - c), 0, (p - a) * (p - b))
            xiB, yiB = x, y
        if p == c:
            xiC, yiC = 0, 0
        else:
            barycentre((p - b) * (p - c), (p - c) * (p - a), 0)
            xiC, yiC = x, y

        # coordonnées des points de contact entre les cercles exinscrit et les
        # côtés du triangle
        ...
        if tt == 1 and gg == 0 and hh == 0 and oo == 0 and ee == 0 and ii == 0 and gn == 0:
            animation_triangle()
        if tt == 0 and gg == 1 and hh == 0 and oo == 0 and ee == 0 and ii == 0 and gn == 0:
            centre_de_gravite()
        if tt == 0 and gg == 0 and hh == 1 and oo == 0 and ee == 0 and ii == 0 and gn == 0:
            ortho_centre()
        if tt == 0 and gg == 0 and hh == 0 and oo == 1 and ee == 0 and ii == 0 and gn == 0:
            cercle_circonscrit()
        if tt == 0 and gg == 0 and hh == 0 and oo == 0 and ee == 1 and ii == 0 and gn == 0:
            Euler()
        if tt == 0 and gg == 0 and hh == 0 and oo == 0 and ee == 0 and ii == 1 and gn == 0:
            cercle_inscrit()
        if tt == 0 and gg == 0 and hh == 0 and oo == 0 and ee == 0 and ii == 0 and gn == 1:
            Gergonne_Nagel()
        if tt == 0 and gg == 0 and hh == 0 and oo == 0 and ee == 0 and ii == 0 and gn == 0 and ss == 1:
            synthese()
        if tt == 0 and gg == 0 and hh == 0 and oo == 0 and ee == 0 and ii == 0 and gn == 0 and ss == 0 and sm == 1:
            Simpson()
...
def animation_triangle():
    ...
    can.create_text(
        150, 500, text="Le triangle a pour sommets A, B, C", fill="white")
    triangle()
...
def triangle():
    ...
    sommet_A = can.create_oval(
        xA - 8, yA - 8, xA + 8, yA + 8, fill="red", width=1)
    sommet_B = can.create_oval(
        xB - 8, yB - 8, xB + 8, yB + 8, fill="red", width=1)
    sommet_C = can.create_oval(
        xC - 8, yC - 8, xC + 8, yC + 8, fill="red", width=1)
...
def centre_de_gravite():
    ...
    triangle()
    mA = can.create_oval(xmA - 3, ymA - 3, xmA + 3,
                         ymA + 3, fill="pink", width=1)
    mB = can.create_oval(xmB - 3, ymB - 3, xmB + 3,
                         ymB + 3, fill="pink", width=1)
    mC = can.create_oval(xmC - 3, ymC - 3, xmC + 3,
                         ymC + 3, fill="pink", width=1)
    marqueur_G = can.create_oval(
        xG - 8, yG - 8, xG + 8, yG + 8, fill="pink", width=1)
...
def ortho_centre():
    ...
    information.config(text=texte)
    can.create_text(
        150, 500, text="L' orthocentre H est à l' intersection \ndes hauteurs issues des sommets A, B, C", fill="white")
    ...
    triangle()
    hA = can.create_oval(xhA - 3, yhA - 3, xhA + 3, yhA +
                         3, fill="light blue", width=1)
    hB = can.create_oval(xhB - 3, yhB - 3, xhB + 3, yhB +
                         3, fill="light blue", width=1)
    hC = can.create_oval(xhC - 3, yhC - 3, xhC + 3, yhC +
                         3, fill="light blue", width=1)
    marqueur_H = can.create_oval(
        xH - 8, yH - 8, xH + 8, yH + 8, fill="light blue", width=1)
...
def cercle_circonscrit():
    ...
    information.config(text=texte)
    can.create_text(
        150, 500, text="Le centre (O) du cercle circonscrit est à  \nl' intersection des médiatrices des\ncôtés BC, CA, AB", fill="white")
    cercle_circonscrit = can.create_oval(
        xO - RO, yO - RO, xO + RO, yO + RO, outline='gold', width=1)
    ...
    triangle()
    mA = can.create_oval(xmA - 3, ymA - 3, xmA + 3,
                         ymA + 3, fill="gold", width=1)
    mB = can.create_oval(xmB - 3, ymB - 3, xmB + 3,
                         ymB + 3, fill="gold", width=1)
    mC = can.create_oval(xmC - 3, ymC - 3, xmC + 3,
                         ymC + 3, fill="gold", width=1)
    centre_cercle_circonscrit = can.create_oval(
        xO - 8, yO - 8, xO + 8, yO + 8, fill="gold", width=1)
...
La droite d' Euler relie les 4 points suivants:   G, H, O, E
    ...
    centre_cercle_Euler = can.create_oval(
        xE - 8, yE - 8, xE + 8, yE + 8, fill="white", width=1)
    cercle_Euler = can.create_oval(
        xE - RE, yE - RE, xE + RE, yE + RE, outline="white", width=1)
    ...
    marqueur_H = can.create_oval(
        xH - 8, yH - 8, xH + 8, yH + 8, fill="light blue", width=1)
    lettre_H = can.create_text(xH, yH, text="H", fill="black")
    aA = can.create_oval(xaA - 3, yaA - 3, xaA + 3,
                         yaA + 3, fill="green", width=1)
    bB = can.create_oval(xbB - 3, ybB - 3, xbB + 3,
                         ybB + 3, fill="green", width=1)
    cC = can.create_oval(xcC - 3, ycC - 3, xcC + 3,
                         ycC + 3, fill="green", width=1)
    mA = can.create_oval(xmA - 3, ymA - 3, xmA + 3,
                         ymA + 3, fill="pink", width=1)
    mB = can.create_oval(xmB - 3, ymB - 3, xmB + 3,
                         ymB + 3, fill="pink", width=1)
...
def cercle_inscrit():
    ...
    information.config(text=texte)
    can.create_text(
        150, 500, text="Le centre (I) du cercle inscrit est à l' intersection\ndes bissectrices issues des sommets    A, B, C", fill="white")
    ...
    cercle_inscrit = can.create_oval(
        xI - RI, yI - RI, xI + RI, yI + RI, outline='yellow', width=1)
    triangle()
    centre_cercle_inscrit = can.create_oval(
        xI - 8, yI - 8, xI + 8, yI + 8, fill="yellow", width=1)
    lettre_I = can.create_text(xI, yI, text="I", fill="black")
    marqueur_iA = can.create_oval(
        xiA - 3, yiA - 3, xiA + 3, yiA + 3, fill="yellow", width=1)
    marqueur_iB = can.create_oval(
        xiB - 3, yiB - 3, xiB + 3, yiB + 3, fill="yellow", width=1)
    marqueur_iC = can.create_oval(
        xiC - 3, yiC - 3, xiC + 3, yiC + 3, fill="yellow", width=1)
    root.after(10, animer)
...
La droite de Nagel relie les 3 points suivants : N, G, I
    ...
    cercle_inscrit = can.create_oval(
        xI - RI, yI - RI, xI + RI, yI + RI, outline='#007030', width=1)
    ...
    triangle()
    droite_Nagel = can.create_line(
        xI, yI, xNagel, yNagel, fill="white", width=1)
    centre_cercle_inscrit = can.create_oval(
        xI - 8, yI - 8, xI + 8, yI + 8, fill="yellow", width=1)
    lettre_I = can.create_text(xI, yI, text="I", fill="black")
    point_de_Gergonne = can.create_oval(
        xGergonne - 8, yGergonne - 8, xGergonne + 8, yGergonne + 8, fill="light blue", width=1)
    lettre_Ge = can.create_text(xGergonne, yGergonne, text="Ge", fill="black")
    point_de_Nagel = can.create_oval(
        xNagel - 8, yNagel - 8, xNagel + 8, yNagel + 8, fill="orange", width=1)
    lettre_N = can.create_text(xNagel, yNagel, text="N", fill="black")
    marqueur_G = can.create_oval(
        xG - 8, yG - 8, xG + 8, yG + 8, fill="pink", width=1)
    lettre_G = can.create_text(xG, yG, text="G", fill="black")
    marqueur_iA = can.create_oval(
        xiA - 3, yiA - 3, xiA + 3, yiA + 3, fill="light blue", width=1)
    marqueur_iB = can.create_oval(
        xiB - 3, yiB - 3, xiB + 3, yiB + 3, fill="light blue", width=1)
    marqueur_iC = can.create_oval(
        xiC - 3, yiC - 3, xiC + 3, yiC + 3, fill="light blue", width=1)
    marqueur_eA = can.create_oval(
        xeA - 3, yeA - 3, xeA + 3, yeA + 3, fill="orange", width=1)
    marqueur_eB = can.create_oval(
        xeB - 3, yeB - 3, xeB + 3, yeB + 3, fill="orange", width=1)
    marqueur_eC = can.create_oval(
        xeC - 3, yeC - 3, xeC + 3, yeC + 3, fill="orange", width=1)
    root.after(10, animer)
...
def Simpson():
    ...
    cercle_circonscrit = can.create_oval(
        xO - RO, yO - RO, xO + RO, yO + RO, outline='gold', width=1)
    marqueur_M = can.create_oval(
        xM - 8, yM - 8, xM + 8, yM + 8, fill="brown", width=1)
    lettre_M = can.create_text(xM, yM, text="M", fill="white")
    marqueur_sA = can.create_oval(
        xMA - 8, yMA - 8, xMA + 8, yMA + 8, fill="brown", width=1)
    lettre_sA = can.create_text(xMA, yMA, text="sA", fill="white")
    marqueur_sB = can.create_oval(
        xMB - 8, yMB - 8, xMB + 8, yMB + 8, fill="brown", width=1)
    lettre_sB = can.create_text(xMB, yMB, text="sB", fill="white")
    marqueur_sC = can.create_oval(
        xMC - 8, yMC - 8, xMC + 8, yMC + 8, fill="brown", width=1)
...
dxA, dyA, dxB, dyB, dxC, dyC = 0.1 * f, 0.1 * \
    f, 0.1 * f, 0.1 * f, 0.1 * f, -0.1 * f
...
information = Label(can2, text="", height=16, width=36, relief=GROOVE,
                    bg="brown", fg="white", justify=LEFT, command=None)
information.pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='Quitter le jeu !', height=1, width=35, relief=GROOVE,
       bg="white", command=quitter).pack(padx=5, pady=5, side=BOTTOM, anchor=SW)
Button(can2, text='Stop !', height=1, width=35, relief=GROOVE, bg="white", activebackground="dark green",
       activeforeground="white", command=stop).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='Figure de synthèse', height=1, width=35, relief=GROOVE, bg="white", activebackground="dark green",
       activeforeground="white", command=synthese).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='Droite de Simpson', height=1, width=35, relief=GROOVE, bg="white", activebackground="dark green",
       activeforeground="white", command=Simpson).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='Points de Gergonne et de Nagel', height=1, width=35, relief=GROOVE, bg="white",
       activebackground="dark green", activeforeground="white", command=Gergonne_Nagel).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='Cercle inscrit', height=1, width=35, relief=GROOVE, bg="white", activebackground="dark green",
       activeforeground="white", command=cercle_inscrit).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='''Cercle et droite d' Euler''', height=1, width=35, relief=GROOVE, bg="white",
       activebackground="dark green", activeforeground="white", command=Euler).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='Cercle circonscrit', height=1, width=35, relief=GROOVE, bg="white", activebackground="dark green",
       activeforeground="white", command=cercle_circonscrit).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='Orthocentre', height=1, width=35, relief=GROOVE, bg="white", activebackground="dark green",
       activeforeground="white", command=ortho_centre).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='Centre de gravité', height=1, width=35, relief=GROOVE, bg="white", activebackground="dark green",
       activeforeground="white", command=centre_de_gravite).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
Button(can2, text='Triangle', height=1, width=35, relief=GROOVE, bg="white", activebackground="dark green",
       activeforeground="white", command=animation_triangle).pack(padx=4, pady=4, side=BOTTOM, anchor=SW)
kmin, kmax, kdef = 0, 10, 1
vitesse_jeu = Scale(can2, length=215, orient=HORIZONTAL, sliderlength=11,
                    label="Vitesse de l'animation    (entre 0 et 10)", from_=kmin, to=kmax, troughcolor='white', command=vitesse)
vitesse_jeu.pack(padx=1, pady=1, side=BOTTOM, anchor=SW)
vitesse_jeu.set(kdef)
ss = Scale(can2, length=215, orient=HORIZONTAL, sliderlength=11,
           label="Dimensionner la fenêtre    (entre 1 et 10)", from_=1, to=10, showvalue=10, command=naruto)
