...
def build_dict(n):
    ...
    dec = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven',
           'twelve', 'thirteen', 'fourteen', 'fifteen', 'sixteen', 'seventeen', 'eighteen', 'nineteen']
    ties = ['twenty', 'thirty', 'forty', 'fifty',
    ...
    for number in numbers:
        if 1 <= number < 20:
            ...
            if index_num == 0:
                ...
                lett_dict[number] = len(
                    ties[index_dec - 2]) + len(dec[index_num - 1])
            ...
            if index_dec == 0:
                ...
                if 1 <= index_dec < 20:
                    lett_dict[number] = len(
                        dec[index_hun - 1]) + len('hundred') + len('and') + len(dec[index_dec - 1])
                elif 20 <= index_dec < 100:
                    index_dec2 = index_dec // 10
                    index_num = index_dec % 10
                    if index_num == 0:
                        lett_dict[number] = len(
                            dec[index_hun - 1]) + len('hundred') + len('and') + len(ties[index_dec2 - 2])
                    else:
                        lett_dict[number] = len(dec[index_hun - 1]) + len('hundred') + len(
                            'and') + len(ties[index_dec2 - 2]) + len(dec[index_num - 1])
        elif number == 1000:
