...
def larg_prod_grid(grid):
    ...
    while row < len(grid):
        while col < len(grid[0]):
            if col > 2:
                up = grid[row][col] * grid[row][col - 1] * \
                    grid[row][col - 2] * grid[row][col - 3]
            if col < len(grid[0]) - 3:
                down = grid[row][col] * grid[row][col + 1] * \
                    grid[row][col + 2] * grid[row][col + 3]
            if row > 2:
                left = grid[row][col] * grid[row - 1][col] * \
                    grid[row - 2][col] * grid[row - 3][col]
            if row < len(grid) - 3:
                right = grid[row][col] * grid[row + 1][col] * \
                    grid[row + 2][col] * grid[row + 3][col]

            if col > 2 and row > 2:
                diag1 = grid[row][col] * grid[row - 1][col - 1] * \
                    grid[row - 2][col - 2] * grid[row - 3][col - 3]
            if col > 2 and row < len(grid) - 3:
                diag2 = grid[row][col] * grid[row + 1][col - 1] * \
                    grid[row + 2][col - 2] * grid[row + 3][col - 3]

            if col < len(grid[0]) - 3 and row > 2:
                diag3 = grid[row][col] * grid[row - 1][col + 1] * \
                    grid[row - 2][col + 2] * grid[row - 3][col + 3]
            if col < len(grid[0]) - 3 and row < len(grid) - 3:
                down = grid[row][col] * grid[row + 1][col + 1] * \
...
def main():
    ...
    assert((grid[6][8], grid[7][9], grid[8][10],
            grid[9][11]) == (26, 63, 78, 14))
